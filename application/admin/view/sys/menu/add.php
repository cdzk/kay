    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-info">
                ...
                <form id="formSubmit" action="{:url('admin/sys.Menu/add')}" method="post">
                    ...
                    <div class="box-body">
                        <div class="form-group">
                            ...
                            <select class="form-control select2" id="menu_parentid" name="menu_parentid" style="width: 100%;">
                                <option {$parent_menu['menu_id'] ?: 'selected="selected"'} value="0">顶部菜单</option>
                                {volist name="menu_list" id="vo"}
                                {if condition="$vo['level'] eq 0"}
                                <option {$parent_menu['menu_id']==$vo.menu_id ? 'selected="selected"' : ''} value="{$vo.menu_id}">{$vo.menu_name}</option>
                                {else/}
                                <option {$parent_menu['menu_id']==$vo.menu_id ? 'selected="selected"' : ''} value="{$vo.menu_id}">├{:str_repeat('─',$vo.level)} {$vo.menu_name}</option>
                                {/if}
                                {/volist}
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="menu_controller" name="menu_controller"
                                   ...
                                   placeholder="控制器名"
                                   value="{$parent_menu['menu_controller']}">
