    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-info">
                ...
                <form id="formSubmit" action="{:url('admin/sys.Auth/add')}" method="post">
                    <input type="hidden" name="__hash__" value="{$Request.token.__hash__}">
                    <div class="box-body">
                        <div class="form-group">
                            <label for="auth_parentid">所属节点</label>
                            <select class="form-control select2" id="auth_parentid" name="auth_parentid" style="width: 100%;">
                                <option {$parent_auth['auth_id'] ?: 'selected="selected"'} value="0">顶级节点</option>
                                {volist name="auth_list" id="vo"}
                                {if condition="$vo['level'] eq 0"}
                                <option {$parent_auth['auth_id']==$vo.auth_id ? 'selected="selected"' : ''} value="{$vo.auth_id}">{$vo.auth_name}</option>
                                {else/}
                                <option {$parent_auth['auth_id']==$vo.auth_id ? 'selected="selected"' : ''} value="{$vo.auth_id}">├{:str_repeat('─',$vo.level)} {$vo.auth_name}</option>
                                {/if}
                                {/volist}
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="auth_controller" name="auth_controller"
                                   ...
                                   placeholder="控制器名"
                                   value="{$parent_auth['auth_controller']}">
