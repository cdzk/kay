                    ...
                    <form id="formSubmit" action="{:url('admin/Auth/save')}" method="post">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="menu_parentid">所属节点</label>
                                <select class="form-control select2" id="auth_parentid" name="auth_parentid" style="width: 100%;">
                                    <option {$parent ?: 'selected="selected"'} value="0">顶级节点</option>
                                    {volist name="auth_list" id="vo"}
                                        {if condition="$vo['level'] eq 0"}
                                            <option {$parent==$vo.auth_id ? 'selected="selected"' : ''} value="{$vo.auth_id}">{$vo.auth_name}</option>
                                        {else/}
                                            <option {$parent==$vo.auth_id ? 'selected="selected"' : ''} value="{$vo.auth_id}">├{:str_repeat('─',$vo.level)} {$vo.auth_name}</option>
                                        {/if}
                                    {/volist}
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="menu_name">权限名称</label>
